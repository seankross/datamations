...
#' @importFrom purrr map map2 map2_dbl map2_dfr map_chr map_dbl map_dfr map_if pmap_dbl pmap_dfr reduce
...
datamation_tibble <- function(pipeline, envir = rlang::global_env(),
  ...
  map(tidy_functions_list, ~ if(!(.x %in% supported_tidy_functions)) {stop(paste(.x, "not supported by datamation_tibble"), call. = FALSE)})

  anim_list <- list()
  dimensions <- list(xmin = xlim[1],
                     xmax = xlim[2],
                     ymin = ylim[1],
                     ymax = ylim[2])
